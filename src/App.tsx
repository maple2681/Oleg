...
export default function App() {
  ...
  const [cars, setCars] = useState<Car[]>(() => {
    try {
      const saved = localStorage.getItem("lacitycars_fleet");
      if (saved) {
        return JSON.parse(saved) as Car[];
      }
    } catch (e) {
      console.error("Failed to parse lacitycars_fleet from localStorage", e);
    }
    return CARS;
  });
  ...
  const confirmDeleteAction = () => {
    ...
    // Instant removal & localStorage update
    setDeleteConfirm(null);

    if (type === "car") {
      setCars((prevCars) => {
        const updatedCars = prevCars.filter((c) => c.id !== id);
        localStorage.setItem("lacitycars_fleet", JSON.stringify(updatedCars));
        return updatedCars;
      });
  ...
  // Save new/edited car
  const handleSaveCar = (e: FormEvent) => {
    e.preventDefault();
    setIsSaving(true);
    ...
    // Update frontend state directly and save to localStorage
    setCars((prev) => {
      const updatedCars = editingCar
        ? prev.map((c) => (c.id === generatedId ? carPayload : c))
        : [carPayload, ...prev];
      try {
        localStorage.setItem("lacitycars_fleet", JSON.stringify(updatedCars));
        showToast("Successfully saved vehicle.");
      } catch (e) {
        if (e instanceof DOMException && e.name === "QuotaExceededError") {
          showToast("Storage limit exceeded. Try smaller images.", "error");
        } else {
          showToast("An error occurred while saving the vehicle.", "error");
        }
      }
      return updatedCars;
    });
